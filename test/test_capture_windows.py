...
def test_windows_hotkey_presets_are_distinct() -> None:
    ...
    assert presets[0].undo.label == "Alt+U"
    assert presets[0].capture.modifiers == MOD_ALT
    assert presets[0].undo.modifiers == MOD_ALT
    assert presets[0].capture.key_code == VK_C
    assert presets[0].undo.key_code == VK_U
    assert presets[1].capture.label == "Control+Alt+C"
    assert presets[1].undo.label == "Control+Alt+Z"
    assert presets[1].capture.modifiers == MOD_CONTROL | MOD_ALT
    assert presets[1].undo.modifiers == MOD_CONTROL | MOD_ALT
    assert all(item.undo.label != "Alt+Z" for item in presets)
