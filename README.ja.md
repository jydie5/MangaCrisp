...
**[Windows 10/11 x64 Development Previewをダウンロード](https://github.com/jydie5/MangaCrisp/releases/download/windows-preview-0.7.0b0.1/MangaCrisp-0.7.0b0-windows-x64-portable-preview.zip)**

未署名のポータブルZIPで、Python、uv、CUDA、インストーラーは不要です。
監査済みのZig版Real-CUGANを同梱し、NVIDIA GPUで操作と補正を確認しています。
0.7ではPDFiumを同梱し、カラーを保持したPDF遅延描画をWindows実機で確認しています。
...
- **macOS連番スクリーンキャプチャ:** 許可された画面範囲を手動操作でカラーPNGへ
...
初回撮影前に[連番スクリーンキャプチャ](#連番スクリーンキャプチャmacosベータ)と
[画面収録の許可手順](INSTALL.ja.md#キャプチャ用の画面収録を許可する)を確認してください。
...
- 表紙を並べる本棚
- 複数アーカイブのドラッグ＆ドロップ登録
- macOSの固定範囲をカラーPNGとCBZ／ZIPへ保存する連番キャプチャ
...
3. `撮影を開始`を押します。初回はmacOSの`画面収録とシステムオーディオ録音`で
   MangaCrispを許可し、アプリを完全終了して同じアプリを開き直します。
4. 対象アプリでページを手動で送り、画像ごとに`Option+C`を1回押します。
   直前の撮影取消は`Option+Z`です。
5. DockのMangaCrispをクリックして管理画面を戻し、`撮影を完了`を押します。
